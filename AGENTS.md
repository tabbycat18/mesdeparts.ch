...
- Frontend stationboard refresh/fetch loop: `realtime_api/frontend/v20260227.main.js`, `realtime_api/frontend/v20260227.logic.js`
...
| Frontend refresh request coalescing (avoid parallel fetch bursts) | `realtime_api/frontend/v20260227.main.js` |
| Frontend foreground refresh/resume drift catch-up + unattended-stall rescue + RT fetch diagnostics (`lastFetchAt`, `edgeCache`, `serverFetchedAt`) | `realtime_api/frontend/v20260227.main.js`, `realtime_api/frontend/v20260227.logic.js`, `realtime_api/frontend/v20260227.state.js` |
| Frontend boot / SW update / bfcache reload | `realtime_api/frontend/v20260227.main.js` (SW_UPDATED message listener + pageshow persisted listener at end of boot) |
...
- Reliable operator detection: use `operatorPatterns` in `network-map.json` matched against `dep.operator` (= `gtfs_agency.agency_name`, e.g. `"Städtische Verkehrsbetriebe Bern"`, `"TPG"`, `"TPN"`). Note: `agency_id` (e.g. `"827"`) is the internal GTFS FK used to JOIN the tables — it is NOT what goes in `operatorPatterns`. Always use `agency_name` (the human-readable string) for matching.
