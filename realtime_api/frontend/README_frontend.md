...
- RT availability notice is driven by backend status and recomputed even on backend `204` responses from the last rendered payload. Mapping: `applied` -> no notice, non-fatal/degraded states (`skipped_budget`, `disabled`, `partial`/`mixed`/`degraded`) -> soft `rtPartiallyAvailable`, hard failure/freshness states (`stale_cache`, `missing_cache`, `guarded_error`, `error`) -> `rtTemporarilyUnavailable`. Legacy fallback (`rt` block) follows the same severity split.
...
- Dual board consumes embedded `boardLoading`/`boardNotice` state so each pane can surface live status in the top banner (localized “Refreshing…”, “Realtime data partially available”, or “Realtime data currently unavailable” depending on RT severity).
