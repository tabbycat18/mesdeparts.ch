...
- RT status is rendered as a discreet persistent indicator (`Live data : •`): label color stays fixed/muted and only the dot color changes (green/orange/red/gray). Classification prefers `meta.rtStatus` when present; fallback is `rt.applied` / `rt.reason`. Mapping: `applied` (or fallback `rt.applied=true` / `rt.reason=fresh`) -> green, `partial|mixed|degraded` -> orange, `missing|error|stale|unavailable` -> red, anything else -> gray. Status is recomputed even on backend `204` responses using the last rendered payload.
